...
public class FolderStructureTreePanel {
            ...
            public void nodeStateChanged(@NotNull CheckedTreeNode node) {
                try {
                    ...
                    if (userObject instanceof VirtualFile) {
                        var length = ((VirtualFile) userObject).getLength();
                        if (node.isChecked()) {
                            totalSize += length;
                        } else {
                            totalSize -= length;
    ...
    private List<VirtualFile> getCheckedVirtualFiles() {
        return Arrays.stream(checkboxTree.getCheckedNodes(VirtualFile.class, node -> !((VirtualFile) node).isDirectory()))
                .collect(toList());
    }
    ...
    private void traverseDirectory(@NotNull CheckedTreeNode parentNode, @NotNull VirtualFile projectDirectory) {
        for (VirtualFile childFile : projectDirectory.getChildren()) {
            ...
            if (!parentNode.isChecked() || isIgnoreFile(childFile)) {
            ...
            public void customizeRenderer(JTree t, Object value, boolean selected, boolean expanded, boolean leaf, int row, boolean focus) {
                ...
                if (userObject instanceof VirtualFile) {
                    ...
                    if (file.isDirectory()) {
                        ...
                        var fileType = FileTypeManager.getInstance().getFileTypeByFile(file);
                        getTextRenderer().setIcon(fileType.getIcon());
                        getTextRenderer().append(" - " + convertFileSize(file.getLength()));
                    }
                }
            }
        };
